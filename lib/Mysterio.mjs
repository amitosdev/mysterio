...
export class Mysterio {
  ...
  async getEnvConfigs() {
    ...
    const envPath = path.join(this.#configDirPath, `${env}.json`)
    const config = await getConfigsFromFile(envPath)
    debug(`getEnvConfigs -> config for env "${env}":`, config)
    return config
  }
  ...
  async getSecrets() {
    if (this.#secretName) {
      ...
      return this.#client(this.#secretName)
    }
    const pkgName = await getPackageName()
    const secretName = `${pkgName}/${process.env.NODE_ENV || 'local'}`
    debug(`getSecrets -> fetching secrets for: ${secretName}`)
    return this.#client(secretName)
  }

  async getMerged(mergingOrder = DEFAULT_MERGING_ORDER) {
